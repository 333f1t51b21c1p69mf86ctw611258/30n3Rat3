...
public class SftpFileDAO {
	...
	public static boolean create(SftpFile sftpFile, boolean warnExisting) {
		...
		String callScript = "{call ? := VNP_COMMON.DML_TABLE.INS_SFTP_FILE(?,?,?,?,?,?,?)}";
		...
		try {
			...
			if (sqlResult == 1) {
				...
				if (warnExisting)
					...
					LOG.debug("WHEN: Insert SftpFile from SLU Folder: " + sftpFile.slu + " => Existed SftpFile on DB: " + sftpFile.sftpFile);
			}
	...
	public static boolean update(SftpFile sftpFile) {
		...
		String callScript = "{call VNP_COMMON.DML_TABLE.UPD_SFTP_FILE(?,?,?,?,?,?,?)}";
