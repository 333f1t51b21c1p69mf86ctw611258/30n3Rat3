...
public abstract class MyFlatFileOutputAdapter
    ...
    private static final DecimalFormat df10 = new DecimalFormat("0000000000");
...
//                                        filePrefix + fileBaseName + fileSuffix;
    ...
    private void prepareHeader() throws ProcessingException {
        ...
        fileHeader.append("\0");
        fileHeader.append(TEMP_HOST_NAME);
        for (int i = 0; i < 33 - TEMP_HOST_NAME.length(); i++) {
            fileHeader.append(" ");
        }

        fileHeader.append("0" + df10.format(startCdrSeq) + "0");
        fileHeader.append(df10.format(startCdrSeq) + "0");
        fileHeader.append(df10.format(beginCdrTime) + "0");
        fileHeader.append(df10.format(beginCdrTime) + "0");
        //		recordCount = 0;
        fileHeader.append(df10.format(recordCount));
...
//  
    ...
    public IRecord prepValidRecord(IRecord r) throws ProcessingException {
        ...
        startCdrSeq++;
        if (startCdrSeq > MAX_CDR_SEQ) {
            startCdrSeq = startCdrSeq - MAX_CDR_SEQ;
        }
        return r;
    }
    ...
    public void updateFileCounter() {
        try {
            ...
            out.println(fileSeq);
            out.println(startCdrSeq + 1);

            out.flush();
            out.close();

        } catch (Exception e) {
            getPipeLog().error("Error opening valid stream output for file " + getSeqFilePath());
        }
    }
    ...
    private String initGetInsertQuery()
            throws InitializationException {
        String tmpFile;
        tmpFile = PropertyUtils.getPropertyUtils().getBatchOutputAdapterPropertyValueDef(getPipeName(), getSymbolicName(),
                SERVICE_INSERT_QUERY_KEY, "");
     ...
     * transaction
     *
     * @param transactionNumber The number of the transaction to get the name
     * for
