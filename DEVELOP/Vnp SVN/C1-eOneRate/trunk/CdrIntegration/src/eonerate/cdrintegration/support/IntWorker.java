...
public class IntWorker implements Runnable {
	...
	private void pullCdrFile(ChannelSftp sftpChannel, String dateFolderName, boolean noteLastest) throws SftpException {
		...
		if (okCd) {
			...
			for (FileItem fileItem : orderFiles) {
				...
				if (!noteLastest || (noteLastest && fileItem.key > sftpLastest.lastestCdrTime)) {
					...
					if (!file.getAttrs().isDir()) {
						...
						try {
							...
							LOG.error("WHEN: Convert seq to number, fileName: " + gzipFileName + "; ERROR: " + e.toString());

						}

						// Neu create SftpFile thanh cong
						boolean okCreate = SftpFileDAO.create(sftpFile, noteLastest);
						if (okCreate) {

							String inputGzipFilePath = inputDateFolderPath + MainConfig.currentInstance.getInputOsFileSeparator() + gzipFileName;

							String outputFolderPath = MainConfig.currentInstance.getDestFolderPath() + Constant.OUTPUT_OS_FILE_SEPARATOR +
									sluName + Constant.OUTPUT_OS_FILE_SEPARATOR +
									dateFolderName;
							...
							if (okDownload) {
								...
								okExtract = FileUtil.extractGzipFile(outputGzipFilePath, extCdrFilePath);
		...
		try {
			...
			if (!MainConfig.currentInstance.isOnlyToday() && hasSluFolderPath(sftpChannel)) {
				...
				for (ListItem dateFolderItem : orderedDateFolderList) {
					...
					pullCdrFile(sftpChannel, dateFolderName, true);
			...
			while (runFlag) {
				...
				if (hasSluFolderPath(sftpChannel)) {
					...
					long tmpMillis = Calendar.getInstance().getTimeInMillis();

					if (tmpMillis - timeAnchor > MainConfig.currentInstance.getSlowerCdrInterval() * 60 * 1000) {
						if (timeAnchor != 0) {
							LOG.info("TODO: Get slower CDR file, slu: " + sluName);
						}

						timeAnchor = tmpMillis;

						pullCdrFile(sftpChannel, yesterday, false);
						pullCdrFile(sftpChannel, today, false);

					} else {

						pullCdrFile(sftpChannel, yesterday, true);
						pullCdrFile(sftpChannel, today, true);
