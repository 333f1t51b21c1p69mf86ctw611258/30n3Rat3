DROP VIEW CBS_OWNER.USAGE_PLAN_VIEW;

/* Formatted on 22/05/2014 15:08:24 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW CBS_OWNER.USAGE_PLAN_VIEW
(
   USAGE_PLAN_NAME,
   USAGE_PLAN_ID,
   OFFER_NAME,
...
AS
   SELECT t7.DISPLAY_VALUE AS usage_plan_name,                -- usagePlanKey,
     ...
     FROM cbs_owner.OFFER_USAGE_PLAN_MAP t1
          ...
          -- *** add usage item
          INNER JOIN cbs_owner.AUT_FINAL_KEY t44 ON t26.AUT_ID = t44.AUT_ID
          INNER JOIN
...
GRANT DELETE, INSERT, SELECT, UPDATE ON CBS_OWNER.USAGE_PLAN_VIEW TO VNP_COMMON;
