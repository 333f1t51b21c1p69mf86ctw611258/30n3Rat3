DROP VIEW CBS_OWNER.OFFER_BALANCE_MAP_VIEW;

/* Formatted on 22/05/2014 15:08:17 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW CBS_OWNER.OFFER_BALANCE_MAP_VIEW
(
   BALANCE_NAME,
   BALANCE_ID,
   OFFER_NAME,
...
AS
     SELECT t4.DISPLAY_VALUE AS balance_name,                   -- balanceKey,
       ...
       FROM cbs_owner.OFFER_BALANCE_MAP t1
            ...
            INNER JOIN cbs_owner.OFFER_KEY t5 ON t1.OFFER_ID = t5.OFFER_ID
            INNER JOIN
...
GRANT DELETE, INSERT, SELECT, UPDATE ON CBS_OWNER.OFFER_BALANCE_MAP_VIEW TO VNP_COMMON;
