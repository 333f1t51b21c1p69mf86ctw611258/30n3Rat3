DROP VIEW CBS_OWNER.USAGE_ACTIVITY_VIEW;

/* Formatted on 22/05/2014 15:08:24 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW CBS_OWNER.USAGE_ACTIVITY_VIEW
(
   UA_ID,
   UA_NAME,
   RESELLER_VERSION_ID,
...
AS
   SELECT t1.AUT_ID AS ua_id,                                        -- autId,
     ...
     FROM cbs_owner.AUT_FINAL_KEY t1
          INNER JOIN cbs_owner.AUT_FINAL_REF t2 ON t1.AUT_ID = t2.AUT_ID
          INNER JOIN
...
GRANT DELETE, INSERT, SELECT, UPDATE ON CBS_OWNER.USAGE_ACTIVITY_VIEW TO VNP_COMMON;
