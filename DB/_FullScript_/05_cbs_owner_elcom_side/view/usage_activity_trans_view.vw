DROP VIEW CBS_OWNER.USAGE_ACTIVITY_TRANS_VIEW;

/* Formatted on 10/12/2014 9:03:08 AM (QP5 v5.215.12089.38647) */
CREATE OR REPLACE FORCE VIEW CBS_OWNER.USAGE_ACTIVITY_TRANS_VIEW
(
   AUT_TRANS_ID,
   UA_NAME,
   UA_ID,
   ...
   SELECT t1.AUT_TRANS_ID,                                   -- AS autTransId,
     ...
     FROM cbs_owner.AUT_TRANSLATION_KEY t1
          ...
          INNER JOIN cbs_owner.RESELLER_VERSION T100
             ON t100.RESELLER_VERSION_ID = t2.RESELLER_VERSION_ID
          ...
          INNER JOIN cbs_owner.AUT_INITIAL_REF t4
             ON     t3.INITIAL_AUT_ID = t4.INITIAL_AUT_ID
                AND t4.SERVICE_VERSION_ID = t100.SERVICE_VERSION_ID
...
GRANT SELECT ON CBS_OWNER.USAGE_ACTIVITY_TRANS_VIEW TO VNP_COMMON;
