/* Formatted on 12/4/2014 22:48:44 (QP5 v5.227.12220.39754) */
SELECT t1.RATE_RC_ID AS rateRcId,
       t1.RESELLER_VERSION_ID AS resellerVersionId,
       t4.DISPLAY_VALUE AS rcTermKey,
  ...
  FROM cbs_owner.RATE_RC t1
       INNER JOIN (SELECT reseller_id, major_version_num, reseller_version_id
                     FROM cbs_owner.reseller_version
                    WHERE reseller_version_id = 2) tReseller
          ON t1.reseller_version_id = tReseller.Reseller_Version_Id
       INNER JOIN
       (SELECT reseller_version_id,
               ...
               minor_version_num,
               reseller_id
          FROM cbs_owner.reseller_version) tBaseVersion
          ON     tReseller.Reseller_Id = tBaseVersion.Reseller_Id
             AND tReseller.Major_Version_Num = tBaseVersion.Major_Version_Num
             AND tBaseVersion.Minor_Version_Num = 0
       INNER JOIN cbs_owner.RC_TERM_KEY t2 ON t1.RC_TERM_ID = t2.RC_TERM_ID
       INNER JOIN
       cbs_owner.RC_TERM_REF t3
          ON     t2.RC_TERM_ID = t3.RC_TERM_ID
             AND t3.RESELLER_VERSION_ID = tBaseVersion.reseller_version_id
       INNER JOIN
       cbs_owner.RC_TERM_VALUES t4
       ...
       LEFT OUTER JOIN
       cbs_owner.EQUIP_TYPE_CODE_VALUES t6
       ...
       LEFT OUTER JOIN
       cbs_owner.EQUIP_CLASS_CODE_VALUES t8
       ...
       LEFT OUTER JOIN
       cbs_owner.CLASS_OF_SERVICE_CODE_VALUES t10
